...
func NewClientConn(c net.Conn, cfg *ClientConfig) (*ClientConn, error) {
	logger.Info("[ws-client] NewClientConn")
	conn := &ClientConn{
		conn:      c,
		config:    cfg,
		Listeners: &common.MultiListener{},
...
func (c *ClientConn) FramebufferUpdateRequest(incremental bool, x, y, width, height uint16) error {
	logger.Info("[ws-client] FramebufferUpdateRequest")
	var buf bytes.Buffer
...
func (c *ClientConn) KeyEvent(keysym uint32, down bool) error {
	logger.Info("[ws-client] KeyEvent")
...
func (c *ClientConn) PointerEvent(mask ButtonMask, x, y uint16) error {
	logger.Info("[ws-client] PointerEvent")
	var buf bytes.Buffer
...
func (c *ClientConn) SetEncodings(encs []common.IEncoding) error {
	logger.Info("[ws-client] SetEncodings")
...
func (c *ClientConn) SetPixelFormat(format *common.PixelFormat) error {
	logger.Info("[ws-client] SetPixelFormat")
...
func parseProtocolVersion(pv []byte) (uint, uint, error) {
	logger.Info("[ws-client] parseProtocolVersion")
	var major, minor uint

	if len(pv) < pvLen {
		return 0, 0, fmt.Errorf("[ws-client] parseProtocolVersion: message too short (%v < %v)", len(pv), pvLen)
	}

	l, err := fmt.Sscanf(string(pv), "RFB %d.%d\n", &major, &minor)
	if l != 2 {
		return 0, 0, fmt.Errorf("[ws-client] parseProtocolVersion: error parsing ProtocolVersion")
	}
	if err != nil {
		return 0, 0, err
	}
...
func (c *ClientConn) handshake() error {
	logger.Info("[ws-client] handshake")
	...
	if maxMajor < 3 {
		return fmt.Errorf("[ws-client] handshake: unsupported major version, less than 3: %d", maxMajor)
	}
	if maxMinor < 8 {
		return fmt.Errorf("[ws-client] handshake: unsupported minor version, less than 8: %d", maxMinor)
	}
	...
	if numSecurityTypes == 0 {
		return fmt.Errorf("[ws-client] handshake: no security types: %s", c.readErrorReason())
	}
...
FindAuth:
	...
	if auth == nil {
		return fmt.Errorf("[ws-client] handshake: no suitable auth schemes found. server supported: %#v", securityTypes)
	}
	...
	logger.Info("[ws-client] handshake: authentication completed successfully")
...
func (c *ClientConn) mainLoop() {
	logger.Info("[ws-client] mainLoop started")
	...
	defer func() {
		logger.Debug("[ws-client] ClientConn.MainLoop: exiting!")
	...
	for {
		...
		if !ok {
			logger.Errorf("[ws-client] ClientConn.MainLoop: bad message type, %d", messageType)
			// Unsupported message type! Bad!
			break
		}
		logger.Infof("[ws-client] ClientConn.MainLoop: got ServerMessage:%s", common.ServerMessageType(messageType))
		...
		if err != nil {
			logger.Errorf("[ws-client] ClientConn.MainLoop: error parsing message, %s", err)
			break
		}
		logger.Infof("[ws-client] ClientConn.MainLoop: read & parsed ServerMessage:%d, %s", parsedMsg.Type(), parsedMsg)
	}
}

func (c *ClientConn) readErrorReason() string {
	logger.Info("[ws-client] readErrorReason")
